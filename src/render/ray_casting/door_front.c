...
/*   Updated: 2025/09/30 12:34:02 by gaeudes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
enum e_hit	only_chech_door_edge(t_ray *ray, t_c_door door)
{
	...
	{
		if (ray->f_coo[Y] - ray->dir[Y] * s_c * (d_x - ray->add_thing[X])
	...
	else if (ray->f_coo[X] - ray->dir[X] * c_s * (d_y- ray->add_thing[Y]) < door->closed_percent + door->x)
	{
