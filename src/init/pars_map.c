...
/*   Updated: 2025/09/18 10:51:54 by gaeudes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	const char allowed_char[])
{
	...
	while (line[i])
	{
		if (!ft_strchr(allowed_char, line[i]))
		{
...
t_err	read_map(t_pars *pars, const char allowed_char[])
{
		...
		&& !pars->error && !pars->error)
	{
		...
		if (check_forbiden_char(pars->rd.line, pars, allowed_char))
			return (pars->error = UNKNOWN_CHAR);
